                ...
                <div class="return-container">
                    ...
                    <div class="filter-section">
                        <div class="search-wrapper">
                            <iconify-icon icon="eva:search-outline" class="search-icon"></iconify-icon>
                            <input type="text" id="idSearchInput" class="search-input" placeholder="Search ID Number or Name...">
                        </div>

                        <div style="display:flex; align-items:center; gap:10px;">
                            <label for="courseFilter" style="font-size:14px; white-space:nowrap;">Filter by Dept:</label>
                            ...
                            <tbody>
                                <?php if ($result && $result->num_rows > 0): ?>
                                    <?php while ($row = $result->fetch_assoc()): ?>
                                        <tr>
                                            <td class="id-cell"><?= htmlspecialchars($row['student_number']) ?></td>
                                            <td class="name-cell"><?= htmlspecialchars($row['borrower_name']) ?></td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function filterTable() {
                const searchValue = idSearchInput.value.toLowerCase().trim();
                const selectedCourse = courseFilter.value;

                tableRows.forEach(row => {
                    const idCell = row.querySelector('.id-cell') ? row.querySelector('.id-cell').textContent.toLowerCase() : '';
                    const nameCell = row.querySelector('.name-cell') ? row.querySelector('.name-cell').textContent.toLowerCase() : '';
                    const courseCell = row.querySelector('.course-cell') ? row.querySelector('.course-cell').textContent.trim() : '';
                    const roleCell = row.querySelector('.role-cell') ? row.querySelector('.role-cell').textContent.trim() : '';

                    // 1. Check Search Match (ID or Name)
                    const matchesSearch = idCell.includes(searchValue) || nameCell.includes(searchValue);

                    // 2. Check Course/Dept Match
                    let matchesCourse = false;
                    if (selectedCourse === 'All') {
                        matchesCourse = true;
                    } else if (selectedCourse === 'Faculty') {
                        if (roleCell === 'Faculty') matchesCourse = true;
                    } else {
                        if (courseCell === selectedCourse) matchesCourse = true;
                    }
            ...
            if (idSearchInput) idSearchInput.addEventListener('input', filterTable);
