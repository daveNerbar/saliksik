...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    // File Upload Logic
    $filePath = isset($_POST['existing_file']) ? $_POST['existing_file'] : '';
    ...
    // ACTION: ARCHIVE (Move to unused_books)
    if ($_POST['action'] == 'archive') {
        $id = $_POST['archive_id'];
        $reason = $conn->real_escape_string($_POST['archive_reason']);

        // 1. Copy to unused_books table
        $copySql = "INSERT INTO unused_books (book_title, authors, isbn, genre, language, total_copies, total_pages, accessor_no, call_number, copyright, volume, edition, publish_year, publisher, description, file_path, reason_for_archiving)
                    SELECT book_title, authors, isbn, genre, language, total_copies, total_pages, accessor_no, call_number, copyright, volume, edition, publish_year, publisher, description, file_path, '$reason'
                    FROM books WHERE id = $id";
        
        if ($conn->query($copySql)) {
            // 2. Delete from main books table
            $conn->query("DELETE FROM books WHERE id = $id");
        }
    }
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .archive-btn { color: #d97706; } /* Amber/Orange for Archive */
