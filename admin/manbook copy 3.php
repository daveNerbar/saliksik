...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'archive') {
        ...
        $copySql = "INSERT INTO unused_books (book_title, authors, isbn, genre, language, ejournal, total_copies, total_pages, accessor_no, call_number, copyright, volume, edition, publish_year, publisher, description, file_path, reason_for_archiving)
                    SELECT book_title, authors, isbn, genre, language, ejournal, total_copies, total_pages, accessor_no, call_number, copyright, volume, edition, publish_year, publisher, description, file_path, '$reason'
...
// A. Get unique genres for the dropdown
$genreQuery = "SELECT DISTINCT genre FROM books ORDER BY genre ASC";
$genreResult = $conn->query($genreQuery);
$availableGenres = [];
while($gRow = $genreResult->fetch_assoc()) {
    if(!empty($gRow['genre'])) {
        $availableGenres[] = $gRow['genre'];
    }
}

// B. Handle Filter Inputs
$filterGenre = $_GET['filter_genre'] ?? 'All';
$filterType = $_GET['filter_type'] ?? 'All'; // E-Journal

// C. Build Query with Filters
$sql = "SELECT * FROM books WHERE 1=1";

if ($filterGenre != 'All') {
    $sql .= " AND genre = '" . $conn->real_escape_string($filterGenre) . "'";
}

if ($filterType != 'All') {
    $sql .= " AND ejournal = '" . $conn->real_escape_string($filterType) . "'";
}
...
<body>
                            ...
                            <tbody id="tableBody">
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        echo "<td>" . ($row['ejournal'] == 'Yes' ? '<span style="color:#00695c; font-weight:bold;">E-Journal</span>' : 'Book') . "</td>"; // Display Type
                                        ...
                                        echo "<td>
                                                ...
                                                <button class='action-btn archive-btn' onclick='archiveBook(" . $row['id'] . ")' title='Archive / Unused'>
                                                    <iconify-icon icon='mdi:archive-arrow-down'></iconify-icon>
                                                </button>
                                                <button class='action-btn delete-btn' onclick='deleteBook(" . $row['id'] . ")' title='Delete Permanent'>
                                    ...
                                    echo "<tr><td colspan='14' style='text-align:center;'>No books found.</td></tr>";
                                }
                                ?>
                            </tbody>
                        </table>
            ...
            <form id="addBookForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group"><label>Genre/Category</label>
                        <input type="text" name="genre" id="genre" placeholder="Enter Genre (e.g. Fiction)" required>
                    </div>
        ...
        <div class="modal-content modal-small">
            ...
            <form id="archiveForm" method="POST">
                ...
                <div class="form-group">
                    ...
                    <textarea name="archive_reason" id="archive_reason" rows="3" required placeholder="e.g., Damaged, Outdated, Lost..."></textarea>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Client-Side Search (for loaded results)
        ...
        function editBook(btn) {
            ...
            document.getElementById('ejournal').value = btn.dataset.ejournal; // Populate E-Journal
