    ...
    if ($checkRes->num_rows > 0) {
        ...
        if ($conn->query($deductSql) === TRUE && $conn->affected_rows > 0) {
            ...
            if ($conn->query($insertSql) === TRUE) {
                echo "<script>
                        alert('Reservation Successful! A copy/access has been held for you.'); 
                        window.location.href='studentviewbooks.php';
                      </script>";
            } else {
                // Rollback deduction if insert fails
                $conn->query("UPDATE books SET total_copies = total_copies + 1 WHERE id = '$bookId'");
                echo "<script>alert('Error creating reservation record.');</script>";
            }
        } else {
            echo "<script>alert('Sorry, this item is currently out of stock / unavailable.');</script>";
        }
    }
}
?>
            ...
            <div class="info-footer">
                ...
                <div class="actions-group">
                    <?php if (isset($book['ejournal']) && $book['ejournal'] == 'Yes'): ?>
                        <div class="ejournal-badge"><i class="fa-solid fa-laptop-file"></i> Electronic Journal Available</div>
                    <?php else: ?>
                    ...
                    <?php if ($book['total_copies'] > 0): ?>
                        <form method="POST" onsubmit="return confirm('Do you want to reserve this item?');">
                            <input type="hidden" name="reserve_book" value="1">
                            <button type="submit" class="reserve-btn"><iconify-icon icon="mdi:calendar-check"></iconify-icon> Reserve This</button>
                        </form>
                    <?php else: ?>
                        <button type="button" class="reserve-btn" disabled style="background-color:#999; cursor:not-allowed;">
                            <iconify-icon icon="mdi:alert-circle"></iconify-icon> Out of Stock
                        </button>
                    <?php endif; ?>
                </div>
            </div>
