...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $genre = $row['genre']; // Assumes your DB has a 'genre' column
...
<body>
    ...
    <script>
        ...
        // Main function to run all filters
        function updateBooks() {
            ...
            sections.forEach(section => {
                ...
                // 1. Check if the whole section matches the category filter
                const isSectionMatch = (category === 'All' || sectionCategory === category);

                if (!isSectionMatch) {
                    section.style.display = 'none';
                    return; // Skip processing cards in this section
                }

                // 2. Filter individual cards within the section
                cards.forEach(card => {
                ...
                // 3. Hide section if no cards are visible
                section.style.display = hasVisibleCards ? 'block' : 'none';
            });

            // Re-run sorting after filtering (optional, but good for UX)
            sortBooks();
        }
        ...
        function sortBooks() {
            ...
            grids.forEach(grid => {
                // Get only visible cards to sort
